...
from tgdcfs.config import GithubRepoConfig
from tgdcfs.core.model import TGFSDirectory, TGFSFileRef, TGFSMetadata
from tgdcfs.core.repository.impl.metadata.github_repo import (
    GithubRepoMetadataRepository,
)
from tgdcfs.core.repository.impl.metadata.github_repo.dir_timestamps import (
    CACHE_VERSION,
    DirTimestampStore,
)
from tgdcfs.core.repository.impl.metadata.github_repo.gh_directory import (
    GithubConfig,
    GithubDirectory,
)
...
def stub_timestamp_store():
    ...
    """
    with patch(
        "tgdcfs.core.repository.impl.metadata.github_repo.DirTimestampStore"
    ) as store_cls:
...
class TestGithubRepoMetadataRepository:
    ...
    @patch("tgdcfs.core.repository.impl.metadata.github_repo.Github")
    ...
    def test_reads_encrypted_and_legacy_names(
        ...
        from tgdcfs.crypto.path_names import (
        ...
        repository = GithubRepoMetadataRepository(mock_github_config, name_key=key)
    ...
    @patch("tgdcfs.core.repository.impl.metadata.github_repo.Github")
    @patch("tgdcfs.core.repository.impl.metadata.github_repo.logger")
    ...
    async def test_push_method(self, mock_github_config):
        ...
        with patch("tgdcfs.core.repository.impl.metadata.github_repo.Github"):
            repository = GithubRepoMetadataRepository(mock_github_config)
...
class TestGithubDirectory:
    ...
    @patch("tgdcfs.core.repository.impl.metadata.github_repo.gh_directory.logger")
...
class TestErrorHandling:
    ...
    def test_file_ref_with_non_numeric_message_id(self, mock_ghc):
        ...
        with patch("tgdcfs.core.repository.impl.metadata.github_repo.Github"):
            ...
            with patch(
                "tgdcfs.core.repository.impl.metadata.github_repo.logger"
            ) as mock_logger:
                root_dir = repository._build_directory_structure()
