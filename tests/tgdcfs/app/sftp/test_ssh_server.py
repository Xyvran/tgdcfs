import pytest
from tgdcfs.app.sftp.server import USER_INFO_KEY, TGFSSSHServer
...
def build_config(mocker, users=None, authorized_keys_dir=None):
    config = mocker.Mock()
    config.tgdcfs.users = users if users is not None else {}
    config.tgdcfs.sftp.authorized_keys_dir = authorized_keys_dir
    return config
...
class TestPasswordAuth:
    ...
    def test_accepts_a_valid_password(self, mocker):
        mocker.patch(
            "tgdcfs.app.sftp.server.authenticate", return_value=AdminUser("admin")
        )
    ...
    def test_rejects_a_wrong_password(self, mocker):
        mocker.patch(
            "tgdcfs.app.sftp.server.authenticate", side_effect=LoginFailed("nope")
        )
        server, conn = build_server(mocker, build_config(mocker))
    ...
    def test_carries_the_readonly_flag(self, mocker):
        mocker.patch(
            "tgdcfs.app.sftp.server.authenticate", return_value=ReadonlyUser("viewer")
        )
        server, conn = build_server(mocker, build_config(mocker))
...
class TestPublicKeyAuth:
    ...
    def test_rejects_a_user_missing_from_the_config(self, mocker, tmp_path, key):
        ...
        config = build_config(mocker, users={}, authorized_keys_dir=str(tmp_path))
