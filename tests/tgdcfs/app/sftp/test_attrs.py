...
from tgdcfs.app.sftp.attrs import dir_attrs, file_attrs, root_attrs


class TestDirAttrs:
    def test_converts_milliseconds_to_seconds(self):
        attrs = dir_attrs(
            created_ms=1_700_000_000_000, modified_ms=1_700_000_123_000, readonly=False
        )
...
class TestFileAttrs:
    ...
    def test_negative_size_is_reported_as_zero(self):
        assert (
            file_attrs(size=-1, created_ms=0, modified_ms=0, readonly=False).size == 0
        )
