...
import asyncssh
import pytest
from tgdcfs.app.sftp.server import USER_INFO_KEY, TGFSSFTPServer
...
def build_file_desc(name: str) -> TGFSFileDesc:
    ...
    fd.add_version(
        TGFSFileVersion(
            id="v1", updated_at=UPDATED_AT, _size=FILE_SIZE, message_ids=[1]
        )
    )
    return fd
...
def build_server(mocker, user=None, clients=None) -> TGFSSFTPServer:
    ...
    config = mocker.Mock()
    config.tgdcfs.sftp.upload_buffer_size_bytes = 1024
    config.tgdcfs.sftp.upload_buffer_dir = None
...
class TestReadWrite:
    ...
    async def test_reading_a_write_handle_is_refused(self, admin):
        handle = await admin.open(b"/notes/new.txt", 0x02 | 0x08, asyncssh.SFTPAttrs())
