...
import asyncssh
import pytest
from tgdcfs.app.sftp.server import make_server_factory, make_sftp_factory
from tgdcfs.config import SFTPConfig
from tgdcfs.core import Client
from tgdcfs.core.model import TGFSDirectory, TGFSFileDesc, TGFSFileVersion
...
def build_config(mocker, users):
    config = mocker.Mock()
    config.tgdcfs.users = users
    config.tgdcfs.sftp = SFTPConfig.from_dict({"upload_buffer_size_mb": 0})
    return config
...
async def server(mocker, channel, users):
    mocker.patch("tgdcfs.core.ops.create_upload_task", new_callable=mocker.AsyncMock)
    mocker.patch(
        "tgdcfs.app.sftp.server.authenticate",
...
def _authenticate(users, username, password):
    from tgdcfs.auth.user import AdminUser, ReadonlyUser
...
class TestTransfers:
    ...
    async def test_an_empty_upload_creates_an_empty_file(
        self, server, channel, tmp_path
    ):
