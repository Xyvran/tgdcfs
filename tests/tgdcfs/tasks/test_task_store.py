...
import asyncio
import datetime
from tgdcfs.tasks.task_store import TaskStore, utcnow
from tgdcfs.tasks.models import TaskStatus, TaskType


class TestUtcNow:
    def test_utcnow(self, mocker):
        mock_datetime = mocker.patch("tgdcfs.tasks.task_store.datetime")
...
class TestTaskStore:
    ...
    async def test_update_task_progress_speed_calculation(
        ...
        mock_utcnow = mocker.patch("tgdcfs.tasks.task_store.utcnow")
...
class TestGlobalTaskStore:
    def test_global_task_store_exists(self):
        ...
        from tgdcfs.tasks.task_store import task_store
