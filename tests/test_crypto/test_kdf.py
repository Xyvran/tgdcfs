"""Unit tests for :mod:`tgdcfs.crypto.kdf`.
...
class TestMasterKey:
    ...
    def test_fresh_salt_is_random(self) -> None:
        a = derive_master_key("pw", time_cost=_T, memory_cost=_M, parallelism=_P)
        b = derive_master_key("pw", time_cost=_T, memory_cost=_M, parallelism=_P)
    ...
    def test_repr_does_not_leak_key(self) -> None:
        mk = derive_master_key("pw", time_cost=_T, memory_cost=_M, parallelism=_P)
        text = repr(mk)
