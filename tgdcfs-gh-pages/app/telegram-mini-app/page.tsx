...
export default function TelegramMiniApp() {
  ...
  useEffect(() => {
    const restoreSession = async () => {
      ...
      setFormData((prev) => ({
        ...prev,
        tgdcfsUrl: savedInfo.tgdcfsUrl ?? "",
        username: savedInfo.username ?? "",
      }));

      if (token && savedInfo.tgdcfsUrl) {
        const client = new WebDAVClient(
          `${savedInfo.tgdcfsUrl}/webdav`,
          token,
          handleError
        );
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch(`${formData.tgdcfsUrl}/login`, {
        method: "POST",
        headers: {
      ...
      await saveSavedInfo({
        tgdcfsUrl: formData.tgdcfsUrl,
        username: formData.username,
      });

      // Create WebDAV client with JWT token
      const client = new WebDAVClient(
        `${formData.tgdcfsUrl}/webdav`,
        token,
        handleError
      );
      ...
      const managerClient = new ManagerClient(`${formData.tgdcfsUrl}/api`, token);
      setManagerClient(managerClient);
      ...
      if (err instanceof errors.CatchableError) {
        ...
        if (
          ...
          !formData.tgdcfsUrl.startsWith("https://")
        ) {
          potentialReason =
            "URL must start with https:// when using secure connections.";
        }
        setError(
          `The URL is not a TGDCFS server, or is not reachable. ${potentialReason}`
        );
      }
    } finally {
      setIsLoading(false);
  ...
  return (
    <ThemeProvider theme={currentTheme}>
      ...
      {!isInTelegram && (
        ...
        >
          <Link
            href="https://t.me/tgdcfsprdbot/manager"
            target="_blank"
      ...
      {isLoggedIn && webdavClient && managerClient ? (
        ...
        >
          <Paper elevation={3} sx={{ p: 4, width: "100%" }}>
            ...
            >
              <TextField
                label="TGDCFS Server URL (without /webdav suffix)"
                value={formData.tgdcfsUrl}
                onChange={handleInputChange("tgdcfsUrl")}
              ...
              <Button
                variant="contained"
                onClick={handleLogin}
                disabled={isLoading || !formData.tgdcfsUrl}
                sx={{ mt: 2 }}
                startIcon={
