...
Unlike the per-upload document-name obfuscation in :mod:`tgdcfs.crypto.names`
...
# Prefix tagging a repo path component as TGDCFS-encrypted.
...
def is_encrypted_path_name(name: str) -> bool:
    """Return whether ``name`` carries the TGDCFS encrypted-path prefix."""
...
def decrypt_path_name(key: bytes, encrypted: str) -> str:
    ...
    if not encrypted.startswith(PATH_NAME_PREFIX):
        raise PathNameEncryptionError("not a TGDCFS-encrypted path name")
