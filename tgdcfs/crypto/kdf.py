"""Key derivation for TGDCFS encryption.
...
The master salt is stored in the TGDCFS config (or a sibling file). It is not
...
# Format constant inherited from tgfs: it is part of the key derivation, so
# it must not follow the project rename or existing files stop decrypting.
...
def fingerprint(master_key: bytes) -> str:
    ...
    Useful for logging which key a TGDCFS instance is operating under without
