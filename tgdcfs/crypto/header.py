"""On-disk file header format for encrypted TGDCFS files.
...
inline (rather than only in the TGDCFS metadata) means a file can be decrypted
purely from its Telegram messages plus the master key, even if the TGDCFS
...
# Magic bytes identifying a TGDCFS encrypted blob. Used to fast-fail on
# non-encrypted or corrupt input.
# The on-disk magic stays the 4-byte tgfs value: files written by tgfs and
# tgdcfs share one format.
MAGIC = b"TGFS"
...
class Algorithm(enum.IntEnum):
    ...
    added here *and* handled in :class:`tgdcfs.crypto.cipher.ChunkedAESGCM`.
    """
