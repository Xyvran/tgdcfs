"""Filename and directory-name obfuscation for TGDCFS.

When the optional ``encrypt_names`` flag is set, the document name that
TGDCFS hands to Telegram on every upload is replaced with a self-tagged,
base64url-encoded AES-256-GCM ciphertext blob. The plaintext name is
still kept inside the TGDCFS metadata.json (which is itself encrypted at
...
# Format constant inherited from tgfs (key derivation input); do not rename.
_HKDF_INFO = b"tgfs-name-key-v1"

# Prefix tagging a Telegram document name as TGDCFS-encrypted. Includes a
...
def is_encrypted_name(name: str) -> bool:
    """Return whether ``name`` carries the TGDCFS encrypted-name prefix."""
...
def decrypt_name(name_key: bytes, encrypted: str) -> str:
    ...
    The normal TGDCFS data path never needs to decrypt filenames because
    the plaintext is already kept in the metadata blob.
    """
    if not encrypted.startswith(NAME_PREFIX):
        raise NameEncryptionError("not a TGDCFS-encrypted name")
