...
* :meth:`save` -- wraps the incoming plaintext ``UploadableFileMessage`` in
  ...
  sizes, which is what TGDCFS persists in its metadata.

* :meth:`get` -- detects per file whether the stored content carries a TGDCFS
  ...
  TGDCFS deployment that turns encryption on later can still read everything
...
from tgdcfs.crypto.header import HEADER_SIZE, MAGIC, FileHeader, InvalidHeaderError
...
if TYPE_CHECKING:
    ...
    # telethon / pyrogram via tgdcfs.core.api.
...
class _HeaderCache:
    ...
    def put(self, version_id: str, entry: Optional[tuple[FileHeader, bytes]]) -> None:
        if version_id in self._store:
            return
...
class EncryptingFileContentRepository(IFileContentRepository):
    ...
    async def get(
        ...
        # Step 1: detect whether this file is encrypted (TGDCFS magic at byte 0)
        ...
        plaintext_total = _plaintext_size_from_ciphertext(fv.size, header.chunk_size)
    ...
    async def _detect(
        ...
        if len(buf) < HEADER_SIZE:
            raise InvalidHeaderError(
                f"file '{name}' starts with TGDCFS magic but is shorter than "
