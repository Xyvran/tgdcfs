...
class ChunkedAESGCM:
    ...
    def encrypt_chunk(self, plaintext: bytes, chunk_index: int) -> bytes:
        ...
        if len(plaintext) > self.chunk_size:
            raise ValueError(f"chunk too large: {len(plaintext)} > {self.chunk_size}")
    ...
    def decrypt_chunk(self, blob: bytes, chunk_index: int) -> bytes:
        ...
        try:
            ...
            raise CipherError(f"authentication failed at chunk {chunk_index}") from exc
...
def chunk_to_ciphertext_offset(chunk_index: int, chunk_size: int) -> int:
    ...
    The caller must add :data:`tgdcfs.crypto.header.HEADER_SIZE` if the
