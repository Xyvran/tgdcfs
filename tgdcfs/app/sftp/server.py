...
from tgdcfs.auth.user import AdminUser, ReadonlyUser, User
from tgdcfs.config import Config
from tgdcfs.core import Client, Clients, Ops
from tgdcfs.core.model import TGFSDirectory
...
from tgdcfs.utils.time import ts
...
def translate_errors(
    ...
    """Turn TGDCFS exceptions into the SFTP status codes clients understand."""
...
class TGFSSSHServer(asyncssh.SSHServer):
    ...
    Password authentication goes through :func:`tgdcfs.auth.basic.authenticate`,
    ...
    listed in ``tgdcfs.users`` so the readonly flag keeps applying.
    """
    ...
    def begin_auth(self, username: str) -> bool:
        if not self._config.tgdcfs.users:
    ...
    def public_key_auth_supported(self) -> bool:
        return bool(self._config.tgdcfs.sftp.authorized_keys_dir)
    ...
    def _user_from_config(self, username: str) -> Optional[User]:
        user_cfg = self._config.tgdcfs.users.get(username)
        if user_cfg is None:
            return None
    ...
    def _load_authorized_keys(
        ...
        directory = self._config.tgdcfs.sftp.authorized_keys_dir
        if not directory:
            return None
...
class TGFSSFTPServer(asyncssh.SFTPServer):
    """Serves the TGDCFS tree over SFTP.
    ...
    def __init__(
        self, chan: asyncssh.SSHServerChannel, clients: Clients, config: Config
    ):
        super().__init__(chan)
    ...
    async def _open_for_write(self, resolved: ResolvedPath, pflags: int) -> WriteHandle:
        ...
        sftp_cfg = self._config.tgdcfs.sftp
        handle = WriteHandle(
            ops=ops,
            path=resolved.relative,
    ...
    async def _rename(self, oldpath: bytes, newpath: bytes, overwrite: bool) -> None:
        ...
        if source.client_name != target.client_name:
            raise asyncssh.SFTPOpUnsupported("Moving between clients is not supported")
